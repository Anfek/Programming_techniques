public class rb_tree<TKeyType extends Comparable<TKeyType>, TDataType>{
    ...
    private int h1(Node<TKeyType, TDataType> node){ //подсчёт высоты дерева
        ...
        if(node.right != null)
            h_right = h1(node.right);
        
        return Math.min(h_left, h_right)+1;
    }
    ...
    private int h2(Node<TKeyType, TDataType> node){ //подсчёт высоты дерева
        int h_left=0, h_right=0;
        if(node == null)
            return 0;
